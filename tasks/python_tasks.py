...
def run_python_dependency_task(repo_dir, repo):
    logger = get_run_logger()
    logger.info(f"Starting Python Dependency analysis for repository: {repo['repo_id']}")
    analyzer = PythonDependencyAnalyzer(logger=logger)
...
def run_python_build_tool_task(repo_dir, repo):
    logger = get_run_logger()
    logger.info(f"Starting Python Build Tool analysis for repository: {repo['repo_id']}")
    analyzer = PythonBuildToolAnalyzer(logger=logger)
    analyzer.run_analysis(
        repo_dir=repo_dir,
        repo=repo
    )
