...
async def get_parent_run_id(flow_name):
    ...
    async with get_client() as client:
        # Retrieve all flows and filter by name
        flows = await client.read_flows()
        matching_flows = [flow for flow in flows if flow.name == flow_name]

        if not matching_flows:
            print(f"No flows found with name: {flow_name}")
            return None

        flow_id = matching_flows[0].id  # Assume first match is correct

        # Get latest flow run for the matched flow ID
