...
@flow(name="batch_repo_subflow", task_runner=ConcurrentTaskRunner(max_workers=5), persist_result=False)
async def batch_repo_subflow(config: FlowConfig, repos: List[Dict]):
    ...
    for batch_num, start_idx in enumerate(range(0, len(repos), batch_size), 1):
        ...
        # Submit each repo task as a tracked Prefect task
        submitted_tasks = [
            safe_process_repo.submit(config=config, repo=repo, parent_run_id=parent_run_id)
            for repo in batch
        ]

        # Gather results
        batch_results = await asyncio.gather(
            *[t.result() for t in submitted_tasks],
