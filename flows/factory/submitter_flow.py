    ...
    while True:
        async with get_client() as client:
            runs = await client.read_flow_runs(
                flow_filter={"deployment_id": {"any_": [processor_deployment]}},
                limit=100
            )
            running = sum(1 for r in runs if r.state.name == "Running")
            waiting = sum(1 for r in runs if r.state.name in ("Scheduled", "Pending"))
            in_flight = running + waiting

            logger.info(f"Flow run status: running={running}, waiting={waiting}, in_flight={in_flight}")

            if in_flight >= MAX_IN_FLIGHT:
                logger.info(f"Max in-flight limit ({MAX_IN_FLIGHT}) reached. Sleeping...")
